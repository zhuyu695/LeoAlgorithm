...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  def filterAccOld(p: Tweet => Boolean, acc: TweetSet): TweetSet = {
    if (p(elem)) {
      acc.incl(elem)
  ...
  def mostRetweeted: Tweet = {
    lazy val lmr = left.mostRetweeted
    lazy val rmr = right.mostRetweeted
    if (!left.empty && lmr.retweets > elem.retweets)
      if (!right.empty && rmr.retweets > lmr.retweets) rmr
      else lmr
    else if (!right.empty && rmr.retweets > elem.retweets) rmr
    else elem
  }
