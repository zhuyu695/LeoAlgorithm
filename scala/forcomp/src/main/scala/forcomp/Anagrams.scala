...
object Anagrams {
  ...
  def wordOccurrences(w: Word): Occurrences = 
    w.toCharArray().groupBy((element: Char) => element.toLower).toSeq.sortBy(_._1).toList.map { case (c: Char, cArray: Array[Char]) => (c, cArray.length) }
  ...
  lazy val dictionaryByOccurrences: Map[Occurrences, List[Word]] = 
    dictionary.map{w => (wordOccurrences(w), w)}.groupBy(_._1).map(kv => (kv._1, kv._2.map { case (o: Occurrences, w: Word) => w }))
  ...
  def combinations(occurrences: Occurrences): List[Occurrences] = occurrences match {
      case Nil => List(List())
      case (ch: Char, oc: Int)::Nil => fixCombination((ch, oc)):::combinations(Nil)
      case (ch: Char, oc: Int)::tail => {
        val tailCombination = combinations(tail)
        val total = for {
          l <- tailCombination
          n <- 1 to oc
        } yield (ch, n) :: l
        return total ::: tailCombination ::: List(List())
      }
    }
    
    def fixCombination(pair: (Char, Int)): List[Occurrences] = pair match {
      case (c, 1) => List(List((c, 1)))
      case (c, n) => List(List((c, n))) ::: fixCombination((c, n - 1))
    }
...
/*  def subtract(x: Occurrences, y: Occurrences): Occurrences = {
    (y.toMap foldLeft x.toMap)(removeTerm).toList
  }
