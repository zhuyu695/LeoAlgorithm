...
object WikipediaSuggest extends SimpleSwingApplication with ConcreteSwingApi with ConcreteWikipediaApi {
  ...
  def top = new MainFrame {
    ...
    val suggestHelperFunc: PartialFunction[Try[List[String]], Unit] = {
        case Success(v) => suggestionList.listData = v
        case Failure(e) => status.text = e.getMessage()
    }
    
    // TO IMPLEMENT
    val suggestionSubscription: Subscription =  suggestions.observeOn(eventScheduler) subscribe(suggestHelperFunc)
    ...
    val pageHelperFunc: PartialFunction[Try[String], Unit] = {
        case Success(v) => editorpane.text = v
        case Failure(e) => status.text = e.getMessage()
    }
    
    // TO IMPLEMENT
    val pageSubscription: Subscription = pages.observeOn(eventScheduler) subscribe(pageHelperFunc)
